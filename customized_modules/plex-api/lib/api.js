...
PlexAPI.prototype.query = function query(options) {
    console.log('The Query Options: ',options);
...
PlexAPI.prototype.perform = function perform(options,final) {
    if (typeof options === 'string') {
        // Support old method of only supplying a single `url` parameter
        options = { uri: options };
    }
    ...
    return this._request(options,final);
};
...
PlexAPI.prototype._request = function _request(options, final) {
     var reqUrl = options.uri;
    if(!final){
         reqUrl = this._generateRelativeUrl(reqUrl);
    }
    ...
    console.log('The final request options: ',reqOpts);
    request(reqOpts, function onResponse(err, response, body) {
        ...
        //return deferred.resolve(Array.from(resolveValue));
    });
//return Array.from(deferred.promise);
...
PlexAPI.prototype._generateRelativeUrl = function _generateRelativeUrl(relativeUrl) {
    console.log('THE URL: ',this.serverUrl);
